...
## Submitted agent: Rotor — breadth-aware rotation momentum

`agent.py` is a no-network, no-LLM strategy built for the live forward-return ranking (Round 2: July 7 – September 4, 2026):

- **Five-state regime ladder (CRASH / OFF / SOFT / ON / FULL):** market health is read from index trend AND breadth (fraction of a 31-name candidate pool above its own 50-day average). A sector rotation — indices broken but breadth alive — is a tradable SOFT/ON state, not a run to cash.
- **Momentum core:** ranks 20 mega-cap leaders + 11 sector ETFs by blended 42/21/10-day momentum and trend gap, minus a volatility penalty; names with a red trailing month are excluded unless a fresh 10-day thrust marks a V-recovery. Top 4 picks, max 2 per correlation cluster (semis / megatech / financials / health / defensives / cyclicals).
- **Convexity sleeve:** small TQQQ (or QLD fallback) allocation only in the FULL state — both indices trending, strong breadth, contained volatility — never as a standing bet.
- **Risk stack:** fast crash brake (3-day index drop or vol explosion → cash, with cooldown), equity-drawdown taper (−6% halves gross, −10% quarters it), ATR-scaled trailing stops (10–20% by name volatility), 2-day trend confirmation before upgrades, hysteresis + thrust override on re-entry.
- **Caps:** per-name targets ≤ 25% (drift-trim at 27.5%), beta-adjusted gross clamped to 1.40x targets with tight trim bands on leveraged names, sell-before-buy, buys capped to cash, ≤ 45 orders/day.
